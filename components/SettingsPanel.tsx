      ...
      <div className="w-80 h-full border p-6 shadow-2xl animate-slide-in-right overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            ...
            className="transition-color hover:bg-(--text-color)/20 cursor-pointer p-1 rounded-md"
          >
            <X size={24} />
          </button>
        </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-2">
              {gradients.map((g, i) => (
                <div
                  key={i}
                  className={`h-12 rounded-lg cursor-pointer ring-2 ${
                    settings.bgColor === g
                      ? "ring-blue-500"
                      : "ring-transparent"
                  }`}
            ...
            <div className="space-y-2">
              ...
              <label className="flex justify-between items-center text-sm">
                ...
                <input
                  type="color"
                  value={
                    settings.editorColor.includes("rgba")
                      ? "#ffffff"
                      : settings.editorColor
                  } // Fallback for rgba
                  onChange={(e) =>
                    updateSettings({ editorColor: e.target.value })
                  }
                  className="cursor-pointer"
                />
              </label>
